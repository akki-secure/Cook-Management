module Gamification
  # EXPイベントの記録とユーザーのexp/levelキャッシュ更新を担う中核サービス
  class ExpGrantService
    # @return [Boolean] 実際にEXPが付与されたか（重複時はfalse）
    ...
    def call
      ActiveRecord::Base.transaction do
        ...
        new_total = user.exp + amount
        new_level = LevelCalculator.level_for_total_exp(new_total)
        user.update!(exp: new_total, level: new_level)

        TitleAssigner.new(user).call
      end
      true
